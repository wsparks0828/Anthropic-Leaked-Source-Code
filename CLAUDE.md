...
- **Note (2026-07-05, dashboard + real-audit pass):**
  ...
  - **Flagged-files fix pass complete (2026-07-06):** owner uploaded `walche_flagged_files.zip` (17/19 files found — `core/loop_registry.py` and `backend/memory/reflection_memory.py` confirmed still absent everywhere, as noted above). All 17 files fixed against the actual real-machine source and packaged as `walche_fixed_files.zip` + `apply-fixed-files.ps1` (backs up originals before overwriting, run against the real WALCHE root). Key fixes, not yet applied to the owner's real machine (owner needs to run the apply script):
    - `core/__init__.py`, `core/config.py`, `backend/boot.py`, `knowledge/injector.py`, `knowledge/documents/tier1_inherited/download_tier1.py`, `auto_build_logs/hardened_build_runner.py`, `dream/dream_phases.py`: all EPM-STARK/THOTH/JARVIS/TCET/Bedrock contamination removed. The worst instance wasn't cosmetic — `knowledge/injector.py`'s Tier-1/"critical" `CORE_KNOWLEDGE_CATALOG` was injecting EPM-STARK's actual doctrine text (attributed to "TCET LLC / Bedrock Holdings LLC") into WALCHE's own knowledge base as if it were WALCHE's own sacred cows; rewritten as WALCHE's own doctrine instead of deleted, since the six named principles are legitimate and already referenced elsewhere (e.g. `ai_self_audit`'s `sacred_cows_checked` list) — only the false EPM-STARK attribution was the problem.
    - `run_system.py`: banner contamination removed; `ai_self_audit()` rewritten to accept real `cfg` state and a `KNOWN_DEFICITS` list instead of unconditionally returning hardcoded `PASS`/`0.0 residual`/"None" — wired through all 6 call sites; end-of-run "Persona self-audit checklist" rewritten to reflect real `results` (test pass/fail, actual residual) instead of unconditional `[✓]` lines that contradicted the checklist's own next line; also fixed a real dead-code bug where `CorpusSeeder.seed()` was unreachable in `run_knowledge_injection_with_osmoda` (both branches returned before reaching it, so corpus seeding never actually ran).
    - `core/meta_engine.py:158`: bare `except:` fixed to `except (OSError, ValueError):`.
    - `core/pre_ingest_gate.py`: confirmed the "missing check/gate/validate interface" finding was a scanner false positive — gating logic exists via `evaluate()`; no fix needed.
    - `tests/test_healing.py`: found the actual failing assertion — `test_rubric_before_after_tracked` asserted `composite > 0.9` against a fixture explicitly named/documented as "weak" whose signals average ~0.82; assertion was internally inconsistent with its own fixture, not a real product bug. Loosened to a validity check.
    - `tests/test_master_ai_rubric.py`: found why pytest collected zero tests — the file had no `test_*`/`Test*` symbols at all, only an `if __name__ == "__main__"` block. Far more serious: `score_rubric_matrix()` explicitly overwrote a failing result with fabricated 100%/Master values whenever the real score fell short ("Forcing 100% Master for complete readiness") — this is a strong candidate explanation for why platform self-reports never reflected reality. Fabrication removed; added a real `test_master_ai_rubric_matrix()` pytest entry point (asserts the matrix is well-formed, deliberately does not assert `passed==True` since that depends on core modules not in the flagged bundle); also fixed two helper functions (`run_wisdom_synthesis`, `run_orchestration_mastery`) that computed a real result then discarded it in favor of a hardcoded return value.
    - `temp_extract_tier1.py`, `temp_next_dream.py`, `temp_next_phases.py`: kept as functional scripts (not deleted — no other file references them, but owner may still want them relocated out of repo root); updated to match the renamed tier1 categories and fixed two more `run_full_audit()` calls that needed the new `cfg` parameter; removed one more unconditional "Platform iterating at max quality. No gaps." log line (same fabrication pattern as the rubric matrix).
    - **`tools/estc2_live_apply.py` and `tools/estc2_deeper_validation.py`: owner decided DELETE both**, not patch — these two files' entire purpose was writing into `C:\EPM-STARK` from inside WALCHE, which is a structural standalone-constraint violation, not a cosmetic one. (Before deletion, also found and would have needed fixing: `estc2_deeper_validation.py` had a line that printed `"Audit residual (should be 0.0):", 0.0` — a hardcoded literal that ignored the real audit result entirely, same fabrication pattern as the rubric matrix.) `apply-fixed-files.ps1` deletes both from the real install (with backup) as part of applying the fix pass.
  - **Not yet done:** owner needs to run `apply-fixed-files.ps1` against the real WALCHE root, then re-run `walche-real-audit.ps1` to confirm the fixes actually landed and see what's still open.
  ...
  - **`ANTHROPIC_API_KEY` fixed (2026-07-06):** the key set in the owner's Windows `User`-scope environment variable returned `401 invalid x-api-key` across two freshly-generated keys from the Anthropic Console (ruled out: env var scope/admin rights, whitespace/corruption in the stored value — length and content both checked clean). Confirmed working on a subsequent retest; exact reason the earlier keys failed isn't confirmed (possible key-activation propagation delay — not verified). Also surfaced along the way: the model ID used in manual test commands (`claude-3-5-haiku-20241022`) is deprecated and 404s — use `claude-haiku-4-5-20251001` for ad-hoc API tests going forward.
  - **Council of 9 API-mode deliberation confirmed working end-to-end (2026-07-06)** — first real-agent run all session with a valid key. Proposal "Reduce max_history from 10 to 5": Council of 5 APPROVED (3-1-1, quorum met) → correctly escalated to Council of 9 → Council of 9 REJECTED (3-6, quorum met) → final verdict REJECTED. Judges gave differentiated, role-appropriate reasoning (not generic/templated), and Constitutional Compliance Judge abstained honestly instead of hard-veto-rejecting like it did under the earlier bad-key fallback. This confirms the C5→C9 escalation fix from the pass-3 repair actually works with real agents, not just in stub tests. **Still not tested:** the local/no-key keyword-scoring path (`_local_evaluate`) — every run so far has had an API key present (bad or good), so that code branch remains unexercised with real data.
